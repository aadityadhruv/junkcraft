...
void player_draw(struct player* player, struct world* world, struct shader* shader) {
    glBindVertexArray(player->graphics._vao_debug);
    ...
    glLineWidth(10.0f);
    glDrawElements(GL_LINES, player->graphics.debug_vertex_count, GL_UNSIGNED_INT, 0);
    glBindVertexArray(0);
}
void player_draw_ui(struct player* player, struct shader* shader) {
    ...
    set_uniform_int("enable_texture", shader, 0);
    // =========== DRAW UI ================
    glBindVertexArray(player->graphics._vao_ui);
    mat4 translate;
    vec4 color = { 0, 0, 0, 1.0f };
    glm_mat4_identity(translate);
    // Scale down crosshair
    set_uniform_mat4("model", shader, translate);
    set_uniform_vec4("color", shader, color);
    glLineWidth(5.0f);
    glDrawElements(GL_LINES, player->graphics.ui_vertex_count, GL_UNSIGNED_INT, 0);
    glBindVertexArray(0);
    // =========== DRAW HOTBAR ================
    float x_unit = 1 / 12.0f;
    for (int i = 0; i < HOTBAR_SIZE; i++) {
        glBindVertexArray(player->graphics._vao_inventory);
        vec4 hb_color = { 0.0f, 0.0f, 0.0f, 0.8f };
        set_uniform_vec4("color", shader, hb_color);
        glm_mat4_identity(translate);
        vec4 pos = { (i + 1) * x_unit * SCREEN_WIDTH, 0.0f, -0.2f, 0.0f };
        glm_translate(translate, pos);
        set_uniform_mat4("model", shader, translate);
        glDrawElements(GL_TRIANGLES, player->graphics.inventory_vertex_count, GL_UNSIGNED_INT, 0);

        float scale = 1.0;
        // Draw a larger box to "highlight" the currently selected hotbar item
        if (i == player->data.curr) {
            scale = 1.0f;
            glm_mat4_identity(translate);
            vec4 pos = { (i + 1) * x_unit * SCREEN_WIDTH, 0.0f, -0.1f, 0.0f };
            glm_translate(translate, pos);
            vec4 hb_selected_color = { 1.0f, 1.0f, 1.0f, 0.8f };
            set_uniform_vec4("color", shader, hb_selected_color);
            set_uniform_mat4("model", shader, translate);
            glDrawElements(GL_TRIANGLES, player->graphics.inventory_vertex_count, GL_UNSIGNED_INT, 0);
        }
        scale = 0.9f;
        float offset = (1 - scale)/2.0f * (1/12.0f) * SCREEN_WIDTH;
        vec4 inbox_pos = { (i + 1) * x_unit * SCREEN_WIDTH + offset, offset, -0.05f, 0.0f };
        glm_mat4_identity(translate);
        glm_translate(translate, inbox_pos);
        glm_scale_uni(translate, scale);
        set_uniform_mat4("model", shader, translate);
        vec4 new_hb_color = { 70 / 255.0f, 70 / 255.0f, 70 / 255.0f, 0.8f };
        set_uniform_vec4("color", shader, new_hb_color);
        glDrawElements(GL_TRIANGLES, player->graphics.inventory_vertex_count, GL_UNSIGNED_INT, 0);
    ...
    set_uniform_int("enable_texture", shader, 1);
    for (int i = 0; i < HOTBAR_SIZE; i++) {
        float scale = 0.9;
        ...
        glm_translate(translate, inbox_pos);
        glm_scale_uni(translate, scale);
        glm_scale_uni(translate, SCREEN_WIDTH/12.0f);
        set_uniform_mat4("model", shader, translate);
        vec4 new_hb_color = { 1.0f, 1.0f, 1.0f, 1.0f };
        set_uniform_vec4("color", shader, new_hb_color);
        enum ITEM_ID hotbar_item = player->data.items[i];
        if (hotbar_item != -1) {
            item_draw(hotbar_item);
        }
    }
    glBindVertexArray(0);
}
...
void player_load_ui(struct player* player) {
    ...
    player->graphics.ui_vertex_count = ARRAY_SIZE(crosshair_draw_order);
    glGenVertexArrays(1, &player->graphics._vao_ui);
    glBindVertexArray(player->graphics._vao_ui);
    create_vbo(&player->graphics._vbo_ui, (void*)crosshair, sizeof(crosshair));
    create_ebo(&player->graphics._ebo_ui, (void*)crosshair_draw_order, sizeof(crosshair_draw_order));
    glEnableVertexAttribArray(0);
    glBindBuffer(GL_ARRAY_BUFFER, player->graphics._vbo_ui);
    glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 0, 0);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, player->graphics._ebo_ui);
    glBindVertexArray(0);

    // === Send hotbar data ====
    player->graphics.inventory_vertex_count = ARRAY_SIZE(hotbar_draw_order);
    glGenVertexArrays(1, &player->graphics._vao_inventory);
    glBindVertexArray(player->graphics._vao_inventory);
    create_vbo(&player->graphics._vbo_inventory, (void*)hotbar_item_vertex, sizeof(hotbar_item_vertex));
    create_ebo(&player->graphics._ebo_inventory, (void*)hotbar_draw_order, sizeof(hotbar_draw_order));
    glEnableVertexAttribArray(0);
    glBindBuffer(GL_ARRAY_BUFFER, player->graphics._vbo_inventory);
    glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 0, 0);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, player->graphics._ebo_inventory);
    glBindVertexArray(0);
}
void player_load_debug(struct player* player) {
    // =============== Face Data ===================
    ...
    player->graphics.debug_vertex_count = ARRAY_SIZE(tmp_order);
    glGenVertexArrays(1, &player->graphics._vao_debug);
    glBindVertexArray(player->graphics._vao_debug);
    // Do the same for debug VBOs and EBOs
    create_vbo(&player->graphics._vbo_debug, (void*)tmp_vertex, sizeof(tmp_vertex));
    create_ebo(&player->graphics._ebo_debug, (void*)tmp_order, sizeof(tmp_order));
    // Enable 3 attribs - position normals texture
    glEnableVertexAttribArray(0);
    // glEnableVertexAttribArray(1);
    // glEnableVertexAttribArray(2);
    // set vao_buffer to pos buffer obj
    glBindBuffer(GL_ARRAY_BUFFER, player->graphics._vbo_debug);
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, player->graphics._ebo_debug);
