...
void engine_start(struct engine* engine) {
    ...
    while (engine->game_loop) {
        ...
        const int num_chunks = (CHUNK_DISTANCE*2 + 1)*(CHUNK_DISTANCE*2 + 1);
        vec3 sorted_chunks[num_chunks];
        int index = 0;
        for (int i = -CHUNK_DISTANCE; i <= CHUNK_DISTANCE; i++) {
            for (int j = -CHUNK_DISTANCE; j <= CHUNK_DISTANCE; j++) {
                int chunk_coord[2] = { engine->curr_chunk[0] + i, engine->curr_chunk[1] + j };
                ...
                sorted_chunks[index][2] = distance;
                index += 1;
            }
        }
        ...
        for (int i = 1; i < num_chunks; i++) {
            for (int j = i; j > 0; j--) {
                if (sorted_chunks[j - 1][2] < sorted_chunks[j][2]) {
                    vec3 tmp;
                    memcpy(tmp, sorted_chunks[j], sizeof(vec3));
                    memcpy(sorted_chunks[j], sorted_chunks[j - 1], sizeof(vec3));
                    memcpy(sorted_chunks[j - 1], tmp, sizeof(vec3));
                }
            }
        }
        for (int i = 0; i < num_chunks; i++) {
            ...
            struct chunk* chunk = {0};
            int chunk_coord[2] = { sorted_chunks[i][0], sorted_chunks[i][1] };
            world_get_chunk_no_gen(engine->world, chunk_coord, &chunk);
